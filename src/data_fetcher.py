...
class DataFetcher:
    ...
    def fetch_data(self, symbol):
        ...
        try:
            self.logger.info(f"Original symbol: {symbol}")
            symbol_kucoin = symbol.replace("-", "/").replace("USD", "USDT:USDT")
            self.logger.info(f"Formatted symbol for KuCoin Futures: {symbol_kucoin}")
            ...
            while since_time < now_time:
                try:
                    ...
                    self.logger.info(f"API response for {symbol_kucoin}: {ohlcv}")
                    if not ohlcv:
                        self.logger.warning(f"No data returned for {symbol_kucoin}. Continuing fetch.")
                        break

                    all_data.extend(ohlcv)
                    last_timestamp = ohlcv[-1][0]
                    if last_timestamp <= since_time:
                        self.logger.warning(f"Stale data detected for {symbol}. Continuing fetch.")
                        break
